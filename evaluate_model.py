# evaluate_model.py
#
# Full evaluation script for the SeER model.
# Computes regression metrics (RMSE, MAE) and ranking metrics (MAP@K, NDCG@K).
#
# Usage:
#   python evaluate_model.py
#   python evaluate_model.py --model seer_model.pth --threshold 3
#   python evaluate_model.py --model seer_model2.pth --full-ranking
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Evaluate a trained SeER model (RMSE, MAE, MAP@K, NDCG@K)"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Interaction threshold for relevancy in author-style metrics (default: 3). "
             "Must match training. WARNING: threshold=1 inflates all ranking metrics."
    )
    parser.add_argument(
        "--k", type=int, default=10,
        help="K for ranking metrics (default: 10)"
    )
    parser.add_argument(
        "--full-ranking", action="store_true",
        help="Also run full-ranking evaluation (scores ALL songs per user). "
             "Much slower but produces proper Precision/Recall/MAP/NDCG."
    )
...
def main():
    ...
    print(f"  Threshold: {INTERACTION_THRESHOLD}")
    print(f"  K:         {K}")
    ...
    print(f"\n[6/6] Computing ranking metrics (author style, threshold={INTERACTION_THRESHOLD})...")
    ...
    map_k = map_at_k(pred_df, K)
    ndcg_k = ndcg_at_k(pred_df, K)

    print(f"\n{'=' * 50}")
    print(f"  RMSE      : {rmse:.4f}")
    print(f"  MAE       : {mae:.4f}")
    print(f"  MAP@{K:<3d}   : {map_k:.4f}  (threshold={INTERACTION_THRESHOLD})")
    print(f"  NDCG@{K:<3d}  : {ndcg_k:.4f}  (threshold={INTERACTION_THRESHOLD})")
    print(f"{'=' * 50}")

    # ==============================================================
    # FULL-RANKING EVALUATION (optional, slower)
    # ==============================================================

    if args.full_ranking:

        print(f"\n[BONUS] Full-ranking evaluation (scoring ALL {num_songs} songs per user)...")
        print("  This computes proper Precision/Recall/MAP/NDCG by ranking the")
        print("  full candidate pool, not just test interactions.\n")
        ...
        print(f"\n{'=' * 50}")
        print(f"  Full-Ranking Results @ K={K}")
        print(f"{'=' * 50}")
